...
const AgricultureCard = ({ tip }) => (
  ...
  ">
    ...
    <div className="flex items-start justify-between gap-2 relative z-10">
      <h3 className="text-[15.5px] font-medium text-[#2C2C2A] leading-snug group-hover:text-[#3B6D11] transition-colors">{tip.title}</h3>
      <Badge status={tip.category} />
    </div>

    <p className="text-[13px] text-[#5F5E5A] leading-relaxed line-clamp-2">{tip.description}</p>

    <Link
      to={`/agriculture/${tip._id}`}
